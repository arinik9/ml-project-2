...
[trAvgTPR_SVM, teAvgTPR_SVM, predTr_SVM, predTe_SVM, trueTr_SVM, trueTe_SVM] = kFoldCrossValidation(y, pcaExpX, 3, learnSVM, predictSVM, computePerformance, 'SVM RBF kernel');

%% Logistic Regression
learnLR = @(y, X) trainNeuralNetwork(y, X, 0, 1, 'sigm', 0, 1e-3, [size(X,2) 2]);
predictLR = @(model, X) predictNeuralNetwork(model, X);

[trAvgTPR_LR, teAvgTPR_LR, predTr_LR, predTe_LR, trueTr_LR, trueTe_LR] = kFoldCrossValidation(y, pcaExpX, 3, learnLR, predictLR, computePerformance, plot_flag, 'Neural Network');
%% Neural Network
learnNN = @(y, X) trainNeuralNetwork(y, X, 0, 1, 'sigm', 0, 1e-3, [size(X,2) 100 2]);
predictNN = @(model, X) predictNeuralNetwork(model, X);

[trAvgTPR_NN, teAvgTPR_NN, predTr_NN, predTe_NN, trueTr_NN, trueTe_NN] = kFoldCrossValidation(y, X, 3, learnNN, predictNN, computePerformance, plot_flag, 'Neural Network');

%% Random Forest
learnRF = @(y, X) trainRandomForest(y, X, 100);
predictRF = @(model, X) predictRandomForest(model, X);

[trAvgTPR_RF, teAvgTPR_RF, predTr_RF, predTe_RF, trueTr_RF, trueTe_RF] = kFoldCrossValidation(y, pcaExpX, 3, learnRF, predictRF, computePerformance, plot_flag, 'RF');
...
[trAvgTPR_GP, teAvgTPR_GP, predTr_GP, predTe_GP, trueTr_GP, trueTe_GP] = kFoldCrossValidation(y, pcaX50, 3, learnGP, predictGP, computePerformance, plot_flag, 'GP');
